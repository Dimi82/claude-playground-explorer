...
Creates a self-contained HTML playground that visualizes the current project's architecture as an interactive concept map. Users mark their knowledge level on each concept, draw connections between concepts, capture ideas, and generate prompts for the brainstorm-refiner agent. All state persists to localStorage.
...
- **localStorage persistence** - all changes auto-save and restore on reload
...
- Title in header
- Topic names and colors
- Preset buttons
- Initial concepts array (from codebase analysis)
- Initial edges array (from relationship mapping)
- Topic options in the idea modal
...
Use this exact structure, replacing the placeholder data with project-specific content:
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .topic-header {
      ...
      background: var(--surface-alt); border-radius: 6px; margin-bottom: 8px; cursor: pointer;
    }
    ...
    .concept-item {
      ...
      border-radius: 4px; cursor: pointer; font-size: 12px; color: var(--text-dim); transition: all 0.15s;
    }
    ...
    .delete-idea-btn {
      ...
      font-size: 14px; padding: 2px 6px; border-radius: 4px; line-height: 1; transition: all 0.15s;
    }
    .delete-idea-btn:hover { background: var(--unknown); color: var(--text); }

    .add-idea-btn {
      width: 100%; background: transparent; border: 1px dashed var(--border);
      color: var(--text-dim); padding: 10px; border-radius: 6px; cursor: pointer;
      font-size: 12px; transition: all 0.15s;
    }
    .add-idea-btn:hover { border-color: var(--accent-dim); color: var(--text); }

    .prompt-area {
      grid-column: 1 / -1; background: var(--surface); padding: 16px 20px;
      display: flex; gap: 16px; align-items: flex-start;
    }
    .prompt-output {
      flex: 1; background: var(--bg); border: 1px solid var(--border); border-radius: 6px;
      padding: 14px; font-family: 'SF Mono', 'Fira Code', monospace; font-size: 12px;
      line-height: 1.5; color: var(--text); min-height: 80px; max-height: 120px; overflow-y: auto;
    }
    .copy-btn {
      background: var(--accent); border: none; color: var(--bg); padding: 12px 24px;
    ...
    .modal input, .modal textarea, .modal select {
      ...
      padding: 10px; border-radius: 4px; font-size: 13px; margin-bottom: 12px;
    }
    ...
    .edge-mode-btn {
      flex: 1; background: var(--surface-alt); border: 1px solid var(--border); color: var(--text-dim);
      padding: 8px 12px; border-radius: 4px; cursor: pointer; font-size: 11px; transition: all 0.15s;
    }
    ...
    .edge-mode-indicator {
      ...
      padding: 6px 12px; border-radius: 4px; font-size: 11px; font-weight: 600; z-index: 50; display: none;
    }
...
<body>
  <div class="container">
    <header>
      ...
      <div class="presets">
        ...
        <!-- Add preset buttons for each topic -->
        <button class="preset-btn" data-preset="topic1">Topic 1</button>
        <button class="preset-btn" data-preset="topic2">Topic 2</button>
        <!-- etc -->
      </div>
    </header>
    ...
    <div class="sidebar">
      <div class="instructions">
        <strong>How to use:</strong> Click concepts to cycle their status (Know → Fuzzy → Unknown → Explore).
        Drag nodes on the canvas. Click "Add Idea" to capture new thoughts.
        The prompt updates automatically based on what you want to explore.
      ...
      <div class="panel-section">
        ...
        <div class="legend">
          <div class="legend-item active" data-status="all">
            <div class="status-badge" style="background: var(--text-dim)"></div>All
          </div>
          <div class="legend-item" data-status="know">
            <div class="status-badge status-know"></div>Know
          </div>
          <div class="legend-item" data-status="fuzzy">
            <div class="status-badge status-fuzzy"></div>Fuzzy
          </div>
          <div class="legend-item" data-status="unknown">
            <div class="status-badge status-unknown"></div>Unknown
          </div>
          <div class="legend-item" data-status="explore">
            <div class="status-badge status-explore"></div>Explore
          </div>
        </div>
      </div>
    ...
    <div class="prompt-area">
      <div class="prompt-output" id="prompt-output">
        Mark concepts as "Explore" or "Unknown" to generate a brainstorm prompt...
      </div>
  ...
  <script>
    ...
    // Topic colors - add one color per topic
    const COLORS = {
      topic1: '#4fc3f7',  // blue
      topic2: '#81c784',  // green
    ...
    // Initial concepts - populate from codebase analysis
    const INITIAL_CONCEPTS = [
      // Topic 1
      { id: 't1-concept1', topic: 'topic1', name: 'Concept Name', desc: 'Description from codebase', status: 'fuzzy', x: 150, y: 120 },
      { id: 't1-concept2', topic: 'topic1', name: 'Another Concept', desc: 'Another description', status: 'unknown', x: 280, y: 80 },
      // Topic 2
      { id: 't2-concept1', topic: 'topic2', name: 'Concept Name', desc: 'Description', status: 'fuzzy', x: 500, y: 100 },
      // etc...
    ];

    // Initial edges - populate from relationship analysis
    const INITIAL_EDGES = [
      { from: 't1-concept1', to: 't1-concept2' },           // same topic
    ...
    function draw() {
      ...
      state.concepts.forEach(concept => {
        ...
        if (concept.status === 'explore') {
          ctx.beginPath();
          ctx.arc(concept.x, concept.y, radius + 8, 0, Math.PI * 2);
        ...
        ctx.fillStyle = COLORS[concept.topic] || COLORS.general;
        ctx.fill();

        // Status ring
        ctx.beginPath();
        ctx.arc(concept.x, concept.y, radius + 3, 0, Math.PI * 2);
        ctx.strokeStyle = STATUS_COLORS[concept.status];
        ctx.lineWidth = 3;
        ctx.stroke();
        ...
        if (isSelected) {
          ctx.beginPath();
          ctx.arc(concept.x, concept.y, radius + 7, 0, Math.PI * 2);
          ctx.strokeStyle = '#fff';
          ctx.lineWidth = 2;
          ctx.stroke();
        }
        ...
        if (words.length <= 2) {
          ctx.fillText(concept.name, concept.x, concept.y + radius + 16);
        } else {
    ...
    function renderSidebar() {
      ...
      // Add general if needed
      if (state.concepts.some(c => c.topic === 'general')) {
        topics.general = 'General / Cross-cutting';
      }
      ...
      Object.entries(topics).forEach(([key, name]) => {
        ...
        dot.style.background = COLORS[key] || COLORS.general;
        ...
        concepts.forEach(c => {
          ...
          if (c.isIdea) {
            ...
            star.textContent = ' ★';
            star.style.color = COLORS[c.topic];
        ...
        conceptGroups.appendChild(group);
      });
    }
    ...
    function renderIdeas() {
      ...
      state.ideas.forEach((idea, idx) => {
        ...
        deleteBtn.addEventListener('click', (e) => {
          ...
          if (idea.id) {
            const ci = state.concepts.findIndex(c => c.id === idea.id);
            if (ci !== -1) state.concepts.splice(ci, 1);
          }
          renderSidebar();
          renderIdeas();
          draw();
        ...
        tag.style.background = (COLORS[idea.topic] || COLORS.general) + '40';
        tag.style.color = COLORS[idea.topic] || COLORS.general;
        tag.textContent = idea.topic;
    ...
    function updatePrompt() {
      ...
      if (explore.length === 0 && unknown.length === 0 && selectedIdeas.length === 0) {
        promptOutput.textContent = 'Mark concepts as "Explore" (purple) or "Unknown" (red) to generate a brainstorm prompt.';
        return;
      }
      ...
      if (explore.length > 0) {
        prompt += '\n\n**I want to actively explore these areas:**';
        explore.forEach(c => prompt += `\n- ${c.name}: ${c.desc}`);
      }
      if (unknown.length > 0) {
        prompt += '\n\n**I need help understanding:**';
        unknown.forEach(c => prompt += `\n- ${c.name}: ${c.desc}`);
      }
      if (selectedIdeas.length > 0) {
        prompt += '\n\n**Specific ideas I want to refine:**';
        selectedIdeas.forEach(i => prompt += `\n- ${i.title}: ${i.desc}`);
      }
    ...
    // ============ CANVAS INTERACTION ============
    function getConceptAt(x, y) {
      return state.concepts.find(c => isVisible(c) && Math.sqrt((c.x - x) ** 2 + (c.y - y) ** 2) < 28);
    }
    ...
    canvas.addEventListener('mousedown', e => {
        ...
        if (concept) {
          if (!state.edgeSource) {
            ...
            const exists = state.edges.some(e =>
              (e.from === state.edgeSource.id && e.to === concept.id) ||
              (e.from === concept.id && e.to === state.edgeSource.id)
            );
            if (!exists) {
              state.edges.push({
                from: state.edgeSource.id, to: concept.id,
                cross: concept.topic !== state.edgeSource.topic, userCreated: true
              });
              saveState();
            }
      ...
      if (state.deleteEdgeMode) {
        const edgeResult = getEdgeNear(x, y);
        if (edgeResult) { state.edges.splice(edgeResult.index, 1); draw(); saveState(); }
        return;
      }

      if (concept) {
        state.dragging = concept;
        state.dragOffset = { x: x - concept.x, y: y - concept.y };
        state.selectedConcept = concept.id;
        renderSidebar();
        draw();
      }
    ...
    deleteEdgeBtn.addEventListener('click', () => {
      state.deleteEdgeMode = !state.deleteEdgeMode;
      state.edgeMode = false;
      state.edgeSource = null;
      deleteEdgeBtn.classList.toggle('active', state.deleteEdgeMode);
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape') {
        state.edgeMode = state.deleteEdgeMode = false;
        state.edgeSource = null;
        edgeModeBtn.classList.remove('active');
        deleteEdgeBtn.classList.remove('active');
        canvasContainer.classList.remove('edge-mode');
        edgeIndicator.classList.remove('visible');
      }
    });
    ...
    document.querySelectorAll('.legend-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        state.filter = item.dataset.status;
        draw();
      });
    });
    ...
    document.getElementById('save-idea').addEventListener('click', () => {
      ...
      if (title && desc) {
        ...
        if (topicNodes.length > 0) {
          ...
          newX = last.x + 80 + Math.random() * 40;
          newY = last.y + 60 + Math.random() * 40;
        }
    ...
    function updatePresetButtons() {
      ...
      container.querySelectorAll('.preset-btn').forEach(btn => existing.add(btn.dataset.preset));
      state.concepts.forEach(c => {
        if (!existing.has(c.topic)) {
          const btn = document.createElement('button');
          btn.className = 'preset-btn';
          btn.dataset.preset = c.topic;
          btn.textContent = TOPIC_NAMES[c.topic] || c.topic;
          btn.addEventListener('click', () => {
            document.querySelectorAll('.preset-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            state.activePreset = btn.dataset.preset;
    ...
    // ============ PERSISTENCE ============
    const STORAGE_KEY = 'brainstorm-explorer-state';

    function saveState() {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({
          concepts: state.concepts, edges: state.edges, ideas: state.ideas, version: 1
        }));
    ...
    function loadState() {
      try {
        ...
        if (saved) {
          ...
          if (p.version === 1) {
            state.concepts = p.concepts || state.concepts;
            state.edges = p.edges || state.edges;
            state.ideas = p.ideas || state.ideas;
            return true;
          }
        }
    ...
    function clearState() {
      if (confirm('Reset playground to default state? This will clear all your changes.')) {
...
When generating a playground for a new project, customize these sections:

1. **`<title>`** - Replace `PROJECT_NAME` with actual project name
2. **`<h1>`** - Same project name in header
3. **Preset buttons** - Add one per topic area discovered
4. **`COLORS`** - One color per topic (use the 5 provided or add more)
5. **`TOPIC_NAMES`** - Display names for each topic
6. **`PROJECT_NAME`** - For the generated prompt text
7. **`INITIAL_CONCEPTS`** - Populate from codebase analysis with real file paths/descriptions
8. **`INITIAL_EDGES`** - Populate from relationship analysis
9. **`<select id="idea-topic">`** - Add options matching your topics

## Critical Rules

1. **Use real codebase elements** - Don't use generic placeholders; find actual file names, component names, patterns
2. **Include file paths in descriptions** - Help user navigate to relevant code (e.g., "Handles batch processing (src/orchestrator.ts)")
...
6. **Open in browser** - Always run `open brainstorm-explorer.html` after creating
